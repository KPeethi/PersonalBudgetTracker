...
def get_expense_insights(expenses: List[Dict[str, Any]], 
    ...
    try:
        ...
        # Generate response from OpenAI with error handling
        try:
            response = openai_client.chat.completions.create(
                model=OPENAI_MODEL,
                messages=[
            ...
            # Log the error but provide fallback insights
            logger.error(f"Error generating expense insights: {str(e)}")
            
            # Create helpful fallback insights based on the expense data
            if not expenses or len(expenses) == 0:
                return "No expense data available for analysis. Start tracking your expenses to get personalized insights!"
